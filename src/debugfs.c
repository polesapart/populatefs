...
int open_filesystem(char *device, char *io_options, blk64_t superblock, blk64_t blocksize)
{

	io_channel data_io = 0;

	if (ext2fs_open2(device, io_options, (EXT2_FLAG_SOFTSUPP_FEATURES | EXT2_FLAG_64BITS | EXT2_FLAG_RW), superblock, blocksize, unix_io_manager, &fs)) {
		fs = NULL;
		return 0;
	}
